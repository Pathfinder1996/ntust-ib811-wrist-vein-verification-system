"""
NTUST-IB811 Contactless Wrist Vein Verification System
(Raspberry Pi 5 + OV5647, Picamera2 version)
Author: Sheng-Yan Dai
"""
...
import cv2
# If you prefer tflite_runtime, swap the import and Interpreter below.
...
main_window.setWindowTitle("NTUST-IB811 Vein Verification System")
...
# Control flag for preview (True = live preview, False = frozen)
ocv = True

def update_frame():
    """Periodic preview update when ocv is True."""
    global ocv
    if not ocv:
        return
...
# ======================= Capture Image =======================
def capture_image():
    """Capture one frame, freeze preview, save to file, show frozen frame."""
    global ocv
    frame = picam2.capture_array()
    if frame is None:
    ...
    # Stop live preview (freeze)
    ocv = False

    # Save captured frame (BGR for OpenCV)
    frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
    save_path = "captured_img.png"
    cv2.imwrite(save_path, frame_bgr)
    print("[Info] Image saved as:", save_path)
...
def process_image():
    ...
    # First defect → P1 (far), P3 (start), P4 (end)
    ...
    img_pil = Image.open("wrist_extraction_roi.png").convert('L').resize((256, 256))
    img_np = np.array(img_pil)
    agc_image = automatic_gamma_correction(img_np, gamma=1.0, is_auto_mode=True)
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(4, 4))
    clahe_image = clahe.apply(agc_image)
    normalized = clahe_image.astype(np.float64) / 255.0
    blurred = cv2.GaussianBlur(normalized, (0, 0), sigmaX=4)
    lap = cv2.Laplacian(blurred, cv2.CV_64F, ksize=1)
    lap = np.maximum(lap, 0.0)
    lap_min, lap_max, _, _ = cv2.minMaxLoc(lap)
    scale_fac = 255.0 / max(-lap_min, lap_max) if max(-lap_min, lap_max) != 0 else 1.0
    final_image = (lap * scale_fac).astype(np.uint8)

    cv2.imwrite("Prediction_img.png", final_image)
    cv2.imwrite("Original.png", img_np)
    ...
    img_roi_display = cv2.imread("Original.png", cv2.IMREAD_GRAYSCALE)
    ...
    img_pred_display = cv2.imread("Prediction_img.png", cv2.IMREAD_GRAYSCALE)
...
# ======================= Model Matching =======================
def match_veins():
    """Compare current Prediction_img.png with a registered user image using TFLite model."""
    # If you use tflite_runtime: interpreter = tflite.Interpreter(model_path=...)
    interpreter = tf.lite.Interpreter(model_path=r"/home/daiyan/test/Ours_model_fold_3.tflite")
    ...
    folder1 = r"/home/daiyan/test"
    folder2 = r"/home/daiyan/test/sign_dataset/"
    ...
    user_image1 = None
    prediction_path = os.path.join(folder1, "Prediction_img.png")
...
# ======================= User Registration =======================
def sign_in_image():
    """Save current Prediction_img.png as a registered user template."""
    save_to_sign_path = r"/home/daiyan/test/sign_dataset"
...
# ======================= Dataset Collection =======================
def save_dataset():
    """Capture 10 frames into wrist_dataset with provided metadata."""
    ...
    folder_path = r"/home/daiyan/test/wrist_dataset/"
