...
儘管有以下更新，這系統仍是個垃圾，各階段演算法皆無法與 SOTA 比較，而且有些地方有重大缺陷，建議去看別人的 Paper...

   - 缺陷 1: 非接觸式手腕感興趣區域 (Region of Interest, ROI) 提取演算法就是個狗屁，在 3 維空間中拍攝 2 維手腕靜脈影像，受測者手腕可任意呈現手部角度、距離與方向，沒有一致的姿勢，提取到的靜脈影像早就變形了。

   - 缺陷 2: 為何手腕 ROI 演算法能在我們收集的 NTUST-IB811 上達到 120 位受測者(實際為 60 位，但左右手可以視為不同人) 平均 0.9775 ± 0.0191 的 SSIM 值 ? 兄弟，仔細看，你會發現，你會訝異，我們的資料集雖然有跨次收集，但都很刻意地讓受測者手腕朝上且維持特定姿勢。真正有挑戰性是受測者能自然地擺放其手腕!!! 但建議還是要有物理對齊的機制(固定式裝置)
...
- `第 5.3.1 章節`之手腕靜脈拍攝裝置：
   ...
   建議改為固定式裝置或在畫面中用引導框的方式拍攝用戶手腕靜脈，即便如此，每個人跨次擺放手腕的位置一定不相同，所以還需套用 ROI 演算法，盡量提取差不多位置的 ROI。(我們的 ROI 演算法依賴幾何輪廓與凸缺陷分析，對手腕旋轉、肌肉狀態等外在條件非常敏感。在非接觸式的情況下只要手腕擺放角度、距離與方向沒有一致，每次提取到的 ROI 會不同，無法保證跨次拍攝穩定性，所以還需透過固定式等方式加強提取到的 ROI 影像穩健性)
